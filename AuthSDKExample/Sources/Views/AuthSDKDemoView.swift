            ...
            .popup(
                    ...
                    case .deleteAccount:
                        DeactivateAccountView(
                            ...
                            onSuccess: {
                                viewModel.presentedPopup = nil
                                viewModel.displayInfo = nil
                                KeychainManager.shared.clearAuthSession()
                            },
                            onFailure: {
                                viewModel.activeAlert = .onFail("Could not delete account. Please try again later.")
                            },
                    ...
                    case .gameServer:
                        GameServerView(
                            ...
                            onUpdatedGameServer: { selectedGameId, gameUUID in
                    ...
                    case .onFail(let message):
                        return Alert(
                            ...
                            dismissButton: .default(Text("OK")) {
                                if (message == "Unauthenticated" || message == "Jwt token already expired") {
                                    viewModel.displayInfo = nil
                                }
                            }
                        )
    ...
    @ViewBuilder private var content: some View {
        VStack(spacing: 20) {
            ...
            VStack (spacing: 16) {
                ...
                button(action: {
                            viewModel.presentedPopup = .deleteAccount
                       },
...
//            }
    ...
    private func popuplinkAccount(onDismiss: @escaping () -> Void?) -> some View {
        return LinkAccountView(
            ...
            onSuccess: { authSession in
                print("link account success \(authSession)")
                viewModel.presentedPopup = nil
            },
