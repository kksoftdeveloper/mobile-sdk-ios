...
class AuthSDKDemoViewModel: ObservableObject {
    ...
    func initSDK() {
        authService.authManager.initSDK(packageName: packageName, appVersion: appVersion)
            .receive(on: DispatchQueue.main)
            .flatMap { [weak self] gameInfo -> AnyPublisher<AuthSessionResponse, any Error> in
                ...
                if versionInfo.forceUpdate {
                    ...
                    // Returning Empty since update is forced; you can adjust this behavior as needed
                    return Empty().eraseToAnyPublisher()
                }
                
                return self.authService.authManager.getAuthSesssion()
            }
